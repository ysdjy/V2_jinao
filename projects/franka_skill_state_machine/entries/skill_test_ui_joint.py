...
"""Joint-action variant of skill_test_ui.py (method B).

Same UI / markers / debug visualization as ``skill_test_ui.py`` but:
  * creates the joint-position env ``Isaac-Stack-Cube-Franka-JointPolicy-v0`` (NOT IK-Abs);
  * every skill emits a JOINT action (grasp/place via internal DLS IK -> q_des; open/close drawer
    via scripted-joint baseline or, optionally, the learned policy);
  * nothing runs until you click a button — the drawer never auto-opens on launch.

The original IK-Abs ``skill_test_ui.py`` is left untouched as the IK-Abs regression entry.
...
from runtime.joint_debug_logger import JointDebugLogger
...
from runtime.simple_scene_layout import SimpleLayoutResult, SimpleSceneLayoutManager
