...
"""Run the Connection open-fridge task with a warp-based state machine.
...
.. code-block:: bash
    ...
    ./isaaclab.sh -p Connection/scripts/state_machine/open_fridge_sm.py --num_envs 4
"""
...
parser = argparse.ArgumentParser(description="Franka open-fridge state machine (Connection).")
parser.add_argument(
...
from connection_tasks.robots.fridge import FRIDGE_DOOR_JOINT
...
class OpenFridgeSm:
    ...
    def __init__(self, dt: float, num_envs: int, device: torch.device | str = "cpu", position_threshold=0.03):
        ...
        self.infront_offset[:, 0] = -0.25
        self.infront_offset[:, 2] = 0.06
