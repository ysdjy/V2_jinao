...
"""Run the Connection open-drawer task with a warp-based state machine.

Mirrors the official ``open_cabinet_sm.py`` logic (REST -> approach -> grasp ->
open -> release), but targets the project's own task ``Connection-Open-Drawer-Franka-IK-Abs-v0``
which loads all assets from ``Connection/assets`` (offline / self-contained).
...
parser = argparse.ArgumentParser(description="Franka open-drawer state machine (Connection).")
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="If > 0, stop after this many environment steps and print a summary (useful for verification/CI). "
...
# Importing the project package registers the Connection-* gym tasks.
import connection_tasks  # noqa: F401
from isaaclab_tasks.utils.parse_cfg import parse_env_cfg

TASK_ID = "Connection-Open-Drawer-Franka-IK-Abs-v0"
...
class OpenDrawerSm:
    """A simple task-space state machine to open a drawer in the cabinet.

    States:
        1. REST: the robot is at rest.
        2. APPROACH_INFRONT_HANDLE: move in front of the handle.
        3. APPROACH_HANDLE: move to the handle.
        4. GRASP_HANDLE: close the gripper on the handle.
        5. OPEN_DRAWER: pull the drawer open.
        6. RELEASE_HANDLE: release the handle (final state).
    """
...
def main():
    ...
    actions[:, 3] = 1.0
    # desired object orientation (only position control of object)
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            # optional bounded run for verification
            step_count += 1
            if args_cli.max_steps > 0 and step_count >= args_cli.max_steps:
                states = open_sm.sm_state.detach().to("cpu").tolist()
                print(f"[open_drawer_sm] reached max_steps={args_cli.max_steps}; per-env SM state = {states}", flush=True)
                print("[open_drawer_sm] (states: 0=REST 1=APPROACH_INFRONT 2=APPROACH 3=GRASP 4=OPEN 5=RELEASE)", flush=True)
                break
