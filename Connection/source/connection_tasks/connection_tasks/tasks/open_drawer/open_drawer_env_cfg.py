...
"""Open-drawer environment configuration (V0).

Scene: a Franka arm in front of a Sektion cabinet, on a ground plane with a dome light.
All heavy assets (robot + cabinet USD) are loaded from the project's local ``assets``
directory, so the task is fully self-contained and reproducible offline.

This mirrors the official ``cabinet`` task structure; the robot and end-effector frame
are populated by the robot-specific config (see ``config/franka``).
"""
...
from connection_tasks.assets_paths import SEKTION_CABINET_USD
...
@configclass
class OpenDrawerSceneCfg(InteractiveSceneCfg):
    """Scene with a robot and a cabinet (abstract; robot + ee_frame set by robot config)."""
    ...
    # frame attached to the top drawer handle (used by the state machine as the grasp target)
...
@configclass
class RewardsCfg:
    """Reward terms for the MDP (used for RL; the state machine ignores them)."""
...
class OpenDrawerEnvCfg(ManagerBasedRLEnvCfg):
    ...
    scene: OpenDrawerSceneCfg = OpenDrawerSceneCfg(num_envs=4096, env_spacing=2.0)
